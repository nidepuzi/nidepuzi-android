...
public class PasswordEditText extends EditText {
    ...
    @DrawableRes
    private int hidePwIcon = R.drawable.eye2;
    ...
    public void initFields(AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        if (attrs != null) {
            TypedArray styledAttributes = getContext().getTheme()
                .obtainStyledAttributes(attrs, R.styleable.PasswordEditText, defStyleAttr,
                    defStyleRes);
            try {
                showPwIcon = styledAttributes.getResourceId(R.styleable.PasswordEditText_iconShow,
                    showPwIcon);
                hidePwIcon = styledAttributes.getResourceId(R.styleable.PasswordEditText_iconHide,
                    hidePwIcon);
            } finally {
    ...
    private void showPasswordVisibilityIndicator(boolean shouldShowIcon) {
        if (shouldShowIcon) {
            Drawable drawable =
                showingPasswordIcon ? ContextCompat.getDrawable(getContext(), hidePwIcon)
